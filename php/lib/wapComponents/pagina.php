...
class Pagina extends WapComponent{
	...
	function Pagina($titulo ){
		...
		$this->cabezal_name = 'cabezal';

		$this->conn = new coneXion("Web", true);
	}

	function setCabezal($src = "cabezal") {
		$this->cabezal_name = $src;
	}
...
//		if(strtoupper($allHeaders["DRUTT"]) == "SI") {
		...
		if(OPERADORA_MCM == 'claro.pe') {
			$tpl_extras_urls = '<a href="http://claroper-cw.qpass.com/ndmns/index">IR a Claro</a>'.
								$tpl_extras_urls;
		}
