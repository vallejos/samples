...
foreach ($listaIds as $i => $contentId) {
	...
	try {
		...
		if ($conectado !== TRUE) $conectado = $ftpCon->login();
		if ($conectado === TRUE) {
			$to = TMP_DIR_PT."/".$tone->getContentFilenameFull();
...
$shellCmd = "cd ".TMP_DIR_PT."; zip -r ../".ZIP_DIR."/$zipFile * ";
$log .= exec ($shellCmd);
