...
class druttScreensaver {
	...
	private $contentType;
	private $dbc;
	private $logFile = "/home/kamus/Web/uy/ancel/drutt-ancel/logs/screensaver.class.log";
...
//      private $mandatoryXMLFields = array("externalId", "deployed", "promotiononly", "startDate", "validUntil", "service", "adult", "name", "title", "tag", "premiumResource");
	...
	function __construct($dbc, $debug=FALSE) {
		...
		$this->service = $servicesDrutt["screensaver"];
...
//		  echo "no+ log";
	...
	public function loadContent($content_id) {
		...
		$sql = "SELECT c.*, cc.descripcion nombreCat, cc.xxx FROM Web.contenidos c INNER JOIN Web.contenidos_cat cc ON (cc.id=c.categoria) WHERE c.id=$this->contentId  AND c.tipo='".WAZZUP_SCREENSAVER."'" ;
		$this->addLog("SQL: ".$sql);
...
//		$this->archivo = str_replace("ss", "gifa", $this->archivo);
		$this->archivo = str_replace("128x128", "650x650", $this->archivo);
	...
	public function genXML() {
		...
		$xml = '<screensaver externalId="'.$this->externalId.'">'."\n";
...
//		echo "<textarea cols=50 rows=10>$this->logmsg</textarea>";
	...
	private function setFilenames() {
		$this->addLog("<".__LINE__."> ".__METHOD__);
		$this->contentFilename = "content_".$this->uniqueId.".gif";
      ...
      private function setExternalId() {
	    ...
	    $this->externalId = PROVIDER_CODE."-".DRUTT_SCREENSAVER."-".$this->dataFormat($this->uniqueId, 5, "0");
