...
class migWallpaper {
	...
	private $contentType;
	private $dbc;
	private $logFile = "/home/kamus/Web/uy/ancel/drutt-ancel/logs/wallpaper.class.log";
...
//      private $mandatoryXMLFields = array("externalId", "deployed", "promotiononly", "startDate", "validUntil", "service", "adult", "name", "title", "tag", "premiumResource");
	...
	function __construct($dbc, $debug=FALSE) {
		...
		$this->service = $servicesDrutt["wallpaper"];
...
//		  echo "no+ log";
	...
	public function loadContent($content_id) {
		...
		$sql = "SELECT c.*, cc.descripcion nombreCat, cc.xxx FROM Web.contenidos c INNER JOIN Web.contenidos_cat cc ON (cc.id=c.categoria) WHERE c.id=$this->contentId  AND c.tipo='".WAZZUP_WALLPAPER."'" ;
		$this->addLog("SQL: ".$sql);
...
//		echo "<textarea cols=50 rows=10>$this->logmsg</textarea>";
	...
	private function setFilenames() {
      ...
      private function setExternalId() {
	    ...
	    $this->externalId = PROVIDER_CODE."-".DRUTT_WALLPAPER."-".$this->dataFormat($this->uniqueId, 5, "0");
