...
class druttRealtone {
	...
	private $contentType;
	private $dbc;
	private $logFile = "/home/kamus/Web/uy/ancel/drutt-ancel/logs/realtone.class.log";
...
//      private $mandatoryXMLFields = array("externalId", "deployed", "promotiononly", "startDate", "validUntil", "service", "adult", "name", "title", "tag", "premiumResource");
	...
	function __construct($dbc, $debug=FALSE) {
		...
		$this->service = $servicesDrutt["realtone"];
...
//		  echo "no+ log";
	...
	public function loadContent($content_id) {
		...
		$sql = "SELECT c.*, cc.descripcion nombreCat, cc.xxx FROM Web.contenidos c INNER JOIN Web.contenidos_cat cc ON (cc.id=c.categoria) WHERE c.id=$this->contentId  AND c.tipo='".WAZZUP_REALTONE."'" ;
		$this->addLog("SQL: ".$sql);
...
//		$this->preview = str_replace("netuy", "contenido", $obj->referencia);
	...
	public function genXML() {
		...
		$xml = '<realtone externalId="'.$this->externalId.'">'."\n";
		...
		$xml .= '<realtonePrelisten>'."\n";
		$xml .= '<variant>'."\n";
		$xml .= '<item uri="'.$this->preview_uri.'" />'."\n";
		$xml .= '</variant>'."\n";
		$xml .= '</realtonePrelisten>'."\n";
		$xml .= '<realtonePremium premiumResource="'.$this->premiumResource.'">'."\n";
		$xml .= '<variant>'."\n";
...
//		echo "<textarea cols=50 rows=10>$this->logmsg</textarea>";
	...
	private function setFilenames() {
      ...
      private function setExternalId() {
	    ...
	    $this->externalId = PROVIDER_CODE."-".DRUTT_REALTONE."-".$this->dataFormat($this->uniqueId, 5, "0");
