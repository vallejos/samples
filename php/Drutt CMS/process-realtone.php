...
if (!is_writable(TMP_DIR_RT)) die("ERROR: ".TMP_DIR_RT." is not writable\n");
else { 
	$log .= "borrando tmp dir\n";
	exec("rm -f ".TMP_DIR_RT."/*");
}
...
foreach ($listaIds as $contentId) {
	...
	try {
		...
		if ($conectado !== TRUE) $conectado = $ftpCon->login();
		if ($conectado === TRUE) {
			$to = TMP_DIR_RT."/".$realtone->getContentFilename();
			...
			if ($bajado === TRUE) {
				...
				$to = TMP_DIR_RT."/".$realtone->getPreviewFilename();
...
if (!$fp = fopen(TMP_DIR_RT."/$xmlFile", "a+")) {
...
$shellCmd = "cd ".TMP_DIR_RT."; zip ../".ZIP_DIR."/$zipFile * ";
$log .= exec ($shellCmd);
