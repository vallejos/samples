...
foreach ($listaIds as $contentId) {
	...
	$fulltrack = new druttFulltrack($dbc, $debug, $marca, $festivo);
	try {
	...
	try {
		...
		$fulltrack->setTag($catdrutt);
                  $fulltrack->setLangs(array_keys($paises_elegidos));
		...
		if ($conectado !== TRUE) $conectado = $ftpCon->login();
		if ($conectado === TRUE) {
			$to = $tmpDir."/".$fulltrack->getContentFilename();
			$from = $fulltrack->getContent();
//			$bajado = copy($from, $to);
$bajado = $ftpCon->bajar($from, $to);
			if ($bajado === TRUE) {
				$log .= "\tdescargando preview...\n";
				$to = $tmpDir."/".$fulltrack->getPreviewFilename();
