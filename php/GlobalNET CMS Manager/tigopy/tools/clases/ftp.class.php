...
class Ftp {
	...
	function crearCarpeta($dirname){
		return @ftp_mkdir( $this->id_conn, $dirname );
	}
	function cambiarAcarpeta($dirname){
		return @ftp_chdir( $this->id_conn, $dirname );
	}
	function borrarCarpeta($dirname){
		return ftp_rmdir( $this->id_conn, $dirname );
	}
	function borrar($file){
		return ftp_delete( $this->id_conn, $file );
	}
	function logout(){
